...
export class FileMemoryStore {
 ...
 applyInTransaction(state,owner,candidates,evidenceIds,{allowReclassify=false}={}) {
 const updates=[];
 for(const candidate of (candidates||[]).slice(0,20)) {
  ...
  if(candidate.id&&(!existing||existing.deleted||existing.userLocked||existing.locked||(!allowReclassify&&existing.facet!==candidate.facet)||(existing.kind==='story')!==story))continue;
  ...
  if(existing&&existing.topic===patch.topic&&existing.facet===patch.facet&&existing.text===patch.text&&existing.readiness===patch.readiness&&existing.summary===patch.summary&&existing.status===patch.status&&existing.strength===patch.strength&&existing.storyType===patch.storyType&&JSON.stringify(existing.evidenceIds)===JSON.stringify(patch.evidenceIds))continue;
  let memory=existing;
