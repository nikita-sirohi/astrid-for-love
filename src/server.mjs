  ...
  return http.createServer(async(req,res)=>{
    ...
    try {
      ...
      if(path.startsWith('/api/')) {
        const actor=req.headers['x-participant-id'];
        const scoped=id=>{if(!actor||actor!==id)throw new AppError('Switch to this participant to access their private view.',403);};
        const needsActor=()=>{if(typeof actor!=='string'||!actor)throw new AppError('Choose a demo participant.',403);};
        let match,result;
        if(method==='GET'&&path==='/api/bootstrap') result=await app.bootstrap();
        else if(method==='GET'&&path==='/api/presenter')result=await app.presenter();
        else if(method==='POST'&&path==='/api/demo/reset')result=await app.reset();
...
export async function start({port=Number(process.env.PORT||4310),mode=process.env.ASTRID_MODE||'live',file}={}) {
  ...
  app.kick();const server=createServer(app);server.requestTimeout=180000;
  try {await new Promise((yes,no)=>{server.once('error',no);server.listen(port,'127.0.0.1',yes);});}
  catch(error){await app.close();await lock.close();await unlink(lockPath);throw error;}
  const close=async()=>{const drained=new Promise(yes=>server.close(yes));await app.close();await drained;await lock.close();await unlink(lockPath);};
  console.log(`Astrid for Love · ${mode==='offline'?'SCRIPTED OFFLINE DEMO':'LIVE ASTRA'} · http://127.0.0.1:${server.address().port}`);
