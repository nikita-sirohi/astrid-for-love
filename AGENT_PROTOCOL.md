...
Memy v0.3.0 adds separate story suggestions, each supported by fresh own user evidence. The application stores them before Astrid replies as private `kind: story` memories with no compatibility facet. Astrid can request a versioned exact-recipient grant for a story ID using the existing permission handoff. Only authorized story content enters an introduction; a promising private matching rationale does not authorize disclosure. Stories are private matching evidence, but remain excluded from readiness and clarification completion. Their presence in Matchy’s pair context does not authorize participant-facing disclosure.
...
Memy understanding records include nullable summary, a display phrase of at most 110 characters. The lore refresh API invokes understand with retrospective=true, the full authorized own transcript after the context cutoff, and current records. It reconciles actual learnings, readiness and summaries; it does not replay profile or clarification updates. Existing unlocked records may cite their own stored evidence; new records require accessible transcript evidence. User locks and tombstones remain authoritative. The commit checks participant freshness, preserves chat history and profile facts, invalidates assessments and pending proposals, and queues matching. A successful commit survives a later model acknowledgement failure.
